...
export default function PostDetails() {
	...
	// Seleciona a imagem principal (medium, thumbnail ou original)
	const mainImage = post?.imagem && post.imagem.length > 0 ? post.imagem[0] : undefined;
	const imageUrl = mainImage?.formats?.medium?.url
		? `${STRAPI_BASE_URL}${mainImage.formats.medium.url}`
		: mainImage?.formats?.large?.url
		? `${STRAPI_BASE_URL}${mainImage.formats.large.url}`
		: mainImage?.formats?.small?.url
		? `${STRAPI_BASE_URL}${mainImage.formats.small.url}`
		: mainImage?.formats?.thumbnail?.url
		? `${STRAPI_BASE_URL}${mainImage.formats.thumbnail.url}`
		: mainImage?.url
		? `${STRAPI_BASE_URL}${mainImage.url}`
		: undefined;
			...
			<div className='w-full max-w-2xl px-4 flex flex-col gap-6'>
				{post?.minibio && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Mini Bio</h2>
						<p className='text-justify text-sm'>{post.minibio}</p>
					</div>
				)}
				{post?.descricaoServicos && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Serviços e Diferenciais</h2>
						<p className='text-justify text-sm whitespace-pre-line'>{post.descricaoServicos}</p>
					</div>
				)}
				{post?.observacao && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Observação</h2>
						<p className='text-justify text-sm whitespace-pre-line'>{post.observacao}</p>
					</div>
				)}
				<div className='bg-white rounded-xl shadow p-4 mb-2'>
				...
				{post?.redesSociais && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Redes Sociais / Contato</h2>
						<p className='text-justify text-sm whitespace-pre-line'>{post.redesSociais}</p>
					</div>
				)}
				{post?.empreendedor && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Empreendedor(a)</h2>
						<p className='text-justify text-sm'>{post.empreendedor}</p>
					</div>
				)}
				...
				{post?.imagem && post.imagem.length > 1 && (
					<div className='bg-white rounded-xl shadow p-4 mb-2'>
						<h2 className='text-lg font-semibold mb-2 border-b pb-1'>Galeria</h2>
				...
				<div className='bg-white rounded-xl shadow p-4 mb-2 text-xs text-gray-500 flex flex-col gap-1'>
					{post?.createdAt && <span>Criado em: {new Date(post.createdAt).toLocaleString()}</span>}
					{post?.updatedAt && <span>Atualizado em: {new Date(post.updatedAt).toLocaleString()}</span>}
					{post?.publishedAt && <span>Publicado em: {new Date(post.publishedAt).toLocaleString()}</span>}
				</div>
			</div>
		</div>
	);
